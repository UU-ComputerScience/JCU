...
interpreterH = restrict loginRedir $ blaze (template interpreterHTML)
    ...
    Just q'  -> do
      ...
      if null errs
        then do
          ...
          writeBS . BS.pack . show $ concat [mkTree proof env | (proof, env) <- enumerateDepthFirst [] result]
  ...
  case mkRule rqrl of
    ...
    Right  rl   -> do
      ...
      case insRes of
        ...
        Nothing      -> error500H "Failed to add rule"
